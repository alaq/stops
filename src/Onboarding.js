...
import { Card, Header, Actions, ContentContainer, GradientBackgrounds } from 'react-native-onboarding-component'
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <GradientBackgrounds colors={pages.map(page => page.backgroundColor)} scrollX={this.scrollX} style={{ height: '56%' }} />
        <Animated.ScrollView
          horizontal
          ref={scrollView => {
        ...
        >
          {pages.map((page, index) => (
            <View key={`pages-${index}`} style={[styles.card, { width: deviceWidth, flexDirection: 'column' }]}>
              <Header>
                <Card scrollX={this.scrollX} index={index} style={{ borderRadius: 0 }}>
                  <Text>Hi</Text>
                  {/* <Lottie
                    ref={animation => {
                      if (animation) {
